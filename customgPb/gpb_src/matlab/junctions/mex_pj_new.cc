                 ...
                 int nrhs, const mxArray *prhs[])
{
	...
	if (mxGetNumberOfElements(prhs[4]) != 19) {
		cout << "There should be 19 params" << endl;
		return;
	}
	...
	unsigned long rad_support1 = static_cast<unsigned long>(in_params[11]);
	unsigned long rad_support2 = static_cast<unsigned long>(in_params[12]);
	double rad_inner_support = in_params[13];
	unsigned long min_n_angles = static_cast<unsigned long>(in_params[14]);
	unsigned long max_n_angles = static_cast<unsigned long>(in_params[15]);
	//threshold parameters
	unsigned long threshold_rad_support = static_cast<unsigned long>(in_params[16]);
	double pos_channel_threshold = in_params[17];
	double norm_power = in_params[18];
	
	
	
	//COMPILE RADIUSES
	array<unsigned long> radii_support(2);
	radii_support(0) = rad_support1;
	radii_support(1) = rad_support2;
	radii_support.sort();
	...
	try {
		lib_image::compute_pj_exp(
			...
			n_slices, n_oris_gpb, radii_support, rad_inner_support,
